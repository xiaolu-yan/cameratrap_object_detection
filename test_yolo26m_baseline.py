"""
YOLO26m Test Script — Baseline
Evaluates the model trained with train_yolo26m_baseline.py.
Usage:
  python test_yolo26m_baseline.py eval
  python test_yolo26m_baseline.py visualize
  python test_yolo26m_baseline.py predict <source>
"""
...
def evaluate(weights: Path):
    ...
    if not weights.exists():
        raise FileNotFoundError(
            ...
            "Run train_yolo26m_baseline.py first, or pass --weights <path/to/best.pt>"
        )
...
def predict(weights: Path, source: str, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "predict" / "yolo26m_baseline")
...
def visualize_test(weights: Path, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "visualize" / "yolo26m_baseline")
    predict(weights, str(test_images), out_dir)


def main():
    parser = argparse.ArgumentParser(description="YOLO26m Camera Trap — Baseline Test")
