"""
YOLOv8n Test Script — Baseline
Evaluates the model trained with train_yolov8n_baseline.py.
Usage:
  python test_yolov8n_baseline.py eval
  python test_yolov8n_baseline.py visualize
  python test_yolov8n_baseline.py predict <source>
"""
...
# Default: best weights from the most recent training run
...
def evaluate(weights: Path):
    ...
    if not weights.exists():
        raise FileNotFoundError(
            ...
            "Run train.py first, or pass --weights <path/to/best.pt>"
        )
    ...
    metrics = model.val(
        ...
        plots=True,       # confusion matrix, PR curve, etc.
        verbose=True,
    )
...
def predict(weights: Path, source: str, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "predict" / "yolov8n_baseline")
...
def visualize_test(weights: Path, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "visualize" / "yolov8n_baseline")
    predict(weights, str(test_images), out_dir)


def main():
    parser = argparse.ArgumentParser(description="YOLOv8n Camera Trap — Test & Inference")
    ...
    pred_p.add_argument("source", type=str,
                        help="Image path, folder, or glob (e.g. 'Second training/images')")
    ...
    vis_p.add_argument("--save-dir", type=Path, default=None,
                       help="Output directory (default: runs/visualize/test)")
