"""
YOLO26m Test Script — Augmented (class balance)
Evaluates the model trained with train_yolo26m_augmented.py.
Usage:
  python test_yolo26m_augmented.py eval
  python test_yolo26m_augmented.py visualize
  python test_yolo26m_augmented.py predict <source>
"""
...
DEFAULT_WEIGHTS = BASE_DIR / "runs" / "train" / "yolo26m_augmented" / "weights" / "best.pt"
...
def evaluate(weights: Path):
    ...
    if not weights.exists():
        raise FileNotFoundError(
            ...
            "Run train_yolo26m_augmented.py first, or pass --weights <path/to/best.pt>"
        )
    ...
    print("\n─── Test Results (yolo26m_augmented) ───────────────────")
...
def predict(weights: Path, source: str, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "predict" / "yolo26m_augmented")
...
def visualize_test(weights: Path, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "visualize" / "yolo26m_augmented")
    predict(weights, str(test_images), out_dir)


def main():
    parser = argparse.ArgumentParser(description="YOLO26m Camera Trap — Augmented Test")
