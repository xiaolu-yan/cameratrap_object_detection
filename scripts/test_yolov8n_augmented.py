"""
YOLOv8n Test Script — Augmented (class balance)
Evaluates the model trained with train_yolov8n_augmented.py.
Usage:
  python test_yolov8n_augmented.py eval
  python test_yolov8n_augmented.py visualize
  python test_yolov8n_augmented.py predict <source>
"""
...
DEFAULT_WEIGHTS = BASE_DIR / "runs" / "train" / "yolov8n_augmented" / "weights" / "best.pt"
...
def evaluate(weights: Path):
    ...
    if not weights.exists():
        raise FileNotFoundError(
            ...
            "Run train_yolov8n_augmented.py first, or pass --weights <path/to/best.pt>"
        )
...
def predict(weights: Path, source: str, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "predict" / "yolov8n_augmented")
...
def visualize_test(weights: Path, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "visualize" / "yolov8n_augmented")
    predict(weights, str(test_images), out_dir)


def main():
    parser = argparse.ArgumentParser(description="YOLOv8n Camera Trap — Exp2 Balanced Test")
