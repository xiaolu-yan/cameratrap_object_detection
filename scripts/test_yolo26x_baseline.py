"""
YOLO26x Test Script — Baseline
Evaluates the model trained with train_yolo26x_baseline.py.
Usage:
  python test_yolo26x_baseline.py eval
  python test_yolo26x_baseline.py visualize
  python test_yolo26x_baseline.py predict <source>
"""
...
DEFAULT_WEIGHTS = BASE_DIR / "runs" / "train" / "yolo26x_baseline" / "weights" / "best.pt"
...
def evaluate(weights: Path):
    ...
    if not weights.exists():
        raise FileNotFoundError(
            ...
            "Run train_yolo26x_baseline.py first, or pass --weights <path/to/best.pt>"
        )
...
def predict(weights: Path, source: str, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "predict" / "yolo26x_baseline")
...
def visualize_test(weights: Path, save_dir: Path | None = None):
    ...
    out_dir = save_dir or (BASE_DIR / "runs" / "visualize" / "yolo26x_baseline")
    predict(weights, str(test_images), out_dir)


def main():
    parser = argparse.ArgumentParser(description="YOLO26x Camera Trap — Baseline Test")
